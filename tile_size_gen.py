...
def roundUpToNearestMultipleOf(num, row_dim):
  ...
  if (remainder != 0):
    return (1200//row_dim) * row_dim + row_dim
  else:
    return num
...
def spaceRemaining(row_dim, reduction_dim):
  ...
  remaining = l1MemoryBytes - outputMatVec - outputFusedAdd - inputFusedAdd - spaceForTiles(row_dim, reduction_dim)
  return remaining
...
def case_1_exhaustive_options(c='n'):
  row_dim_no_padding = list(filter(lambda x: dividesIntoOutputVectorEltCount(x), rowDimOptions()))
  reduction_dim_no_padding = list(filter(lambda x: dividesIntoInputVectorEltCount(x), reductionDimOptions()))
  exhaustive = list(product(row_dim_no_padding, reduction_dim_no_padding))
  if c=='p':
    print(f'case_1_exhaustive_options')
    print(f'\tlen {len(row_dim_no_padding)}; row_dim_no_padding: {row_dim_no_padding}')
    print(f'\tlen {len(reduction_dim_no_padding)}; reduction_dim_no_padding: {reduction_dim_no_padding}')
  return exhaustive
...
def case_1_pruned_options(c='n'):
  ...
  if (inputVectorEltCount % 2) != 0:
    print(f"WARNING: M = {inputVectorEltCount} is NOT divisible by 2!")
  reduction_dim_halve_options_for_double_buffering = list(filter(lambda x: x <= (inputVectorEltCount//2)+1, reduction_dim_no_padding))
  greaterThan20 = list(filter(lambda x: x > 20, reduction_dim_halve_options_for_double_buffering))
  pruned = list(product(row_dim_no_padding, greaterThan20))
  if c == 'p':
    print('case_1_pruned_options')
    print(f'\tlen {len(row_dim_no_padding)}; row dim unchanged: {row_dim_no_padding}')
    print(f'\tlen {len(reduction_dim_halve_options_for_double_buffering)}; reduction dims pruned for double buffering: {reduction_dim_halve_options_for_double_buffering}')
    print(f'\tlen {len(greaterThan20)}; pruned away reduction dims < 20: {greaterThan20}')
  return pruned
...
def case_2_exhaustive_options(c='n'):
  ...
  # STRICTLY CASE 2
  reduction_dim_no_padding = list(filter(lambda x: dividesIntoInputVectorEltCount(x), reductionDimOptions()))
  reduction_dim_w_o_case_1 = list(set(reductionDimOptions()).difference(set(reduction_dim_no_padding)))
  exhaustive = list(product(row_dim_no_padding, reduction_dim_w_o_case_1))
  if c=='p':
    print(f'case_2_exhaustive_options')
    print(f'\tlen {len(row_dim_no_padding)}; row_dim_no_padding: {row_dim_no_padding}')
    print(f'\tlen {len(reduction_dim_w_o_case_1)}; reduction_dim_w_o_case_1: {reduction_dim_w_o_case_1}')
  return exhaustive
...
def case_2_pruned_options(c='n'):
  ...
  if (inputVectorEltCount % 2) != 0:
    print(f"WARNING: M = {inputVectorEltCount} is NOT divisible by 2!")
  reduction_dim_halve_options_for_double_buffering = list(filter(lambda x: x <= (inputVectorEltCount//2)+1, reduction_dim_w_o_case_1))
  ...
  for i in range(0,len(reduction_dim_halve_options_for_double_buffering)):
    if (i % 10) == 0:
      reduction_dim_every_tenth.append(reduction_dim_halve_options_for_double_buffering[i])
  ...
  if c=='p':
    ...
    print(f'\tlen {len(row_dim_no_padding)}; row dim left unchanged: {row_dim_no_padding}')
    print(f'\tlen {len(reduction_dim_w_o_case_1)}; reduction dim with case 1 pruned away: {reduction_dim_w_o_case_1}')
    print(f'\tlen {len(reduction_dim_halve_options_for_double_buffering)}; reduction dims pruned for double buffering: {reduction_dim_halve_options_for_double_buffering}')
    print(f'\tlen {len(reduction_dim_every_tenth)}; For dispatch 1, kept every 10th reduction dim: {reduction_dim_every_tenth}')
    print(f'\tlen {len(reduction_dim_every_tenth)}; For dispatch 1, pruned away reduction dims < 20: {greaterThan20}')
...
def case_3_exhaustive_options(c='n'):
  ...
  if c=='p':
    ...
    print(f'\tlen {len(row_dim_w_o_case_1)}; row_dim_w_o_case_1: {row_dim_w_o_case_1}')
    print(f'\tlen {len(reduction_dim_no_padding)}; reduction_dim_no_padding: {reduction_dim_no_padding}')
...
def case_3_pruned_options(c='n'):
  row_dim_no_padding = list(filter(lambda x: dividesIntoOutputVectorEltCount(x), rowDimOptions()))
  row_dim_less_than_or_equal_to_112 = list(filter(lambda x: x >= 112, rowDimOptions()))
  w_o_case_1 = list(set(row_dim_less_than_or_equal_to_112).difference(set(row_dim_no_padding)))
  ...
  if c == 'p':
   ...
   # print(f'\tlen {len(row_dim_less_than_or_equal_to_112)}; pruned away row_dims > 112: {row_dim_less_than_or_equal_to_112}')
    print(f'\tlen {len(w_o_case_1)}; pruned away row dims from case 1: {w_o_case_1}')
   # print(f'\tlen {len(everyOther)}; For dispatch 8, pruned away every other row dim: {everyOther}')
    print(f'\tlen {len(reduction_dim_halve_options_for_double_buffering)}; reduction dims pruned for double buffering: {reduction_dim_halve_options_for_double_buffering}')
    print(f'\tlen {len(greaterThan20)}; For dispatch 1, 9, pruned away reduction dims <= 20: {greaterThan20}')
  return pruned
...
def annotateOption(tup):
  return (tup, spaceForTiles(tup[0],tup[1]), weightMatTileSize(tup[0],tup[1]), spaceRemaining(tup[0],tup[1]))

# helper for converting to CSV
def convertAnnotationToFlatTuple(elt):
  return (f'{0}-{elt[0][0]}-{elt[0][1]}',elt[0][0],elt[0][1],elt[1],elt[2],elt[3])

# helper for converting to CSV
def annotationColumnNames():
  columns=['JSON Name','Row Dim','Reduction Dim','Space Needed in L1','Weight Matrix Tile Size','Space Remaining']
  return columns

# export annotated options to CSV
def exportOptionsToCSV(caseNo, options):
  as_flat_tuples=list(map(convertAnnotationToFlatTuple, options))
  #print(as_flat_tuples)
  df = pd.DataFrame(tuple(as_flat_tuples), columns=annotationColumnNames())
  df['Case']=caseNo
  df.to_csv(f'myrtle_tile_size_gen_out/{dispatchName}_case{caseNo}_searchSpace.csv',index=False)
  print(f'Saved CSV to ./myrtle_tile_size_gen_out/{dispatchName}_case{caseNo}_searchSpace.csv')

def case1():
  print("\nCase 1: No padding in either dimension. --------------------------------|",end='')
  print("\n                                                                        |")
  print("Exhaustive:")
  all_options = case_1_exhaustive_options('p')
  annotated_options=list(map(lambda tup: annotateOption(tup), all_options))
  valid_options = list(filter(lambda tup: smallEnough(tup[0][0], tup[0][1]),annotated_options))
...
def case2():
  print("\nCase 2: Padding in column dimension ONLY. ------------------------------|", end='')
  print("\n                                                                        |")
  print("Exhaustive:")
...
def case3():
  print("\nCase 3: Padding in row dimension ONLY. -----------------------------------|",end='')
  print("\n                                                                          |")
  print("Exhaustive:")
  ...
  print("__________________________________________________________________________|")
# def case1():
#   print("Case 1: No padding in either dimension.")
#   print("Exhaustive:")
#   all_options = case_1_exhaustive_options()
#   annotated_options=list(map(lambda tup: annotateOption(tup), all_options))
#   valid_options = list(filter(lambda tup: smallEnough(tup[0][0], tup[0][1]),annotated_options))
#   print(f'{len(valid_options)} options: {valid_options}')
#   print("Pruned:")
#   pruned_options = case_1_pruned_options()
#   annotated_options=list(map(lambda tup: annotateOption(tup), pruned_options))
#   valid_options = list(filter(lambda tup: smallEnough(tup[0][0], tup[0][1]),annotated_options))
#   print(f'{len(valid_options)} options: {valid_options}')
#   exportOptionsToCSV(2, valid_options)
...
def main():
    ...
    print("\n|----------- CREATING TILE SIZE/SHAPE SEARCH SPACE FOR KERNEL ----------|",end='')
    print("\n|                                                                       |",end='')
    print(f'\n\t\t{dispatchName}')
