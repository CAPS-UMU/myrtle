...
def look_for_enclosing_loop(module: ModuleOp, output = StringIO()) -> (EnclosingSCFLoop,str):
    mulsBefore = []
    mulsAfter = []
    sawFrep = False
    frepCount = 0
...
def check_lowered_matvec_tiling(firstDim : int, outputVectorEltCount : int, inputVectorEltCount : int):
    ...
    linalg_mod, asm_mod, m, n, k = createMatmulTransposeB(firstDim, outputVectorEltCount, inputVectorEltCount)
    # look for a hardware loop
    loops, commentary = look_for_frep(asm_mod)
    if len(loops) != 1:
        raise Exception(f'ERROR: expected only ONE hardware loop, but found {len(loops)} {commentary}')
...
# Dependencies: Python3, xdsl
# python3 -m venv myenv
# source myenv/bin/activate
# pip install xdsl[riscv]
