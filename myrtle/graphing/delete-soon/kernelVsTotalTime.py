...
# from inside myrtle/myrtle, run python3 -m graphing.kernelVsTotalTime
...
def main():
    ...
    print(titles)
    mode = args[1]
    print('FYI: For kernel vs full NN time, we ignore the tile selection mode {mode}.')
    #load dispatch data
    path = "/home/emily/myrtle/myrtle/accuracy/with-old-data"
    myPath = lambda d,mode,case: f"{path}/dispatch_{d}_case{case}_everything-myrtle-{mode}-ranking.csv"
    
    dfs = loadDFsDispatchCaseNo(args[0], dispatchOrder, caseNos, dispatcheSizes, mode, myPath)
    ...
    # generate graph dispatch times vs whole NN time using OLD DATA
    graphKernelVsTotalTime('graphing/out/KernelVsTotalTime-3-dispatches-old-data.png',tup)
    graphActualTime('graphing/out/TotalTime-3-dispatches-rank-x-axis.png',"rank","Kernel Time Rank","fastest to slowest",tup)
    
    # generate single graph for new data
    dispatchOrder = [7,7,7]
    # myPath = lambda d,mode,case: f"{args[0]}/dispatch-{d}-rerun.csv"
    # = lambda d,mode,case: f"{args[0]}/dispatch-{d}-rerun-old-subset.csv"
    #csv_experiment_results-right-600x400-old-subset
    # myPath = lambda d,mode,case: f"{args[0]}/csv_experiment_results-right-600x400-old-subset.csv"
    
    myPath = lambda d,mode,case: f"{args[0]}/csv_experiment_results-probably-right-600x600-old-subset.csv"
    dfs = loadDFsDispatchCaseNo(args[0], dispatchOrder, caseNos, dispatcheSizes, mode, myPath)
    # derive more data about each dispatch
    deriveMoreData2(dfs,dispatchOrder,caseNos,mode)
    tup=dfs,dispatchOrder,caseNos,titles
    graphKernelVsTotalTime('graphing/out/KernelVsTotalTime-single-dispatch-subset-new-data.png',tup)
    graphActualTime('graphing/out/TotalTime-single-dispatch-subset-new-data.png',"rank","Kernel Time Rank","fastest to slowest",tup)
    
    print("HASTA LUEGO")


def graphKernelVsTotalTime(path,tup):
    dfs,dispatchOrder,caseNos,titles=tup
    keys = Keys2D(
            x="rank",
            x_label="Kernel Time Rank",
    ...
    y2 = "Total Time"
    kernelVsTotalTimeDispatchCase(dfs, dispatchOrder, caseNos, titles,y1=keys.y,y2=y2,keys=keys,imgTitle="",imgName=path)
    
def loadDFsDispatchCaseNo(path, dispatchNos, caseNos, inputSizes, mode, myPath):
    dfs = {}
...
def kernelVsTotalTimeDispatchCase(dfs, dispatchNos, caseNos, titles,y1="actualColName",y2="predColName",keys=Keys2D(
            x="rank",
            x_label="Kernel Time Rank",
            x_unit="fastest to slowest",
            y="Time",
            y_label="Time",
            y_unit="cycles",
        ),imgTitle="Kernel vs Total Time",imgName='KernelVsTotal-3-dispatches.png'):
    graphs = []
    for dispNo in dispatchNos:
        for caseNo in caseNos:
            title = titles[dispNo]    
            lftGraph = kernelVsTotalTime(dfs, dispNo, caseNo, title,y1,y2,keys)
            graphs.append(lftGraph)
            graphEmAll((1, 1), [lftGraph])
    combineDispatchesWithContext(graphs,imgTitle,imgName)
    

def kernelVsTotalTime(dfs, dispNo, caseNo, title, y1="actualColName",y2="predColName",keys=Keys2D(
            x="rank",
            x_label="Kernel Time Rank",
            x_unit="fastest to slowest",
            y="Time",
    ...
    tableData = dfs[(dispNo,caseNo)][["rankAsStr","Row Dim","Reduction Dim","Kernel Time", "Total Time"]]
    colLabels = ["rank","n","k", "Kernel Time", "Total Time"]
    ...
    return Graph2D(
        imagePath=f'graphing/out/KernelVsTotal-dispatch-{dispNo}-case-{1}',
        keys=keys,
        title=title,
        scatterSets=[
            (
...
def graphActualTime(path,x,x_label,x_unit,tup):
    ...
    keysActual = Keys2D(
            ...
            y_label="Time", #y_label="Kernel Time",
            y_unit="cycles",
        )
