from tile_static_analysis.utils import (
...
class TSA_Quidditch(TileSizeAnalyzer):
    ...
    def simulate_peek_at_lowered_matmul_tiling(self, cc_tile: MatmulInputs):
        ...
        if not res:
            print(
                f"{cc_tile}:expected FMADD is {expectedFMADDs} but got {(hLoop.body_size * hLoop.loop_iters) * oLoop.iters*ooLoop.iters} instead"
            )
            print("\t", end="")
            print(hLoop)
            print("\t", end="")
    ...
    def getLoweringInfoAnnotation(self, mat: MatmulInputs, sizes: TileSizes):
        ...
        a_operand_ssr_reuse_loads = (
            (hLoop.body_size - 1) * hLoop.loop_iters * oLoop.iters * ooLoop.iters
        )
        a_operand_ssr_start_reuse_loads = (
            1 * hLoop.loop_iters * oLoop.iters * ooLoop.iters
        )
        b_operand_ssr_loads = (
            hLoop.body_size * hLoop.loop_iters * oLoop.iters * ooLoop.iters
        )
        total_ssr_loads_per_core = (
            (hLoop.body_size * 2) * hLoop.loop_iters * oLoop.iters * ooLoop.iters
        )
        assert total_ssr_loads_per_core == (
            a_operand_ssr_reuse_loads
            + a_operand_ssr_start_reuse_loads
            + b_operand_ssr_loads
        )
        ...
        a_operand_ssr_start_reuse_loads = (
            a_operand_ssr_start_reuse_loads * cc_tile_count
        )
