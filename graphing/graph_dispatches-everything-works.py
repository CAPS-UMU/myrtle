...
def graphXvsYrankedbyZtopQ(dfs,x,x_unit, y,y_unit,z,lowIsGoodZ,dispatchNo,dispatchTitle,q):
    # rankings
    ranked = rankBy(dfs,(dispatchNo,1),z,lowIsGoodZ)
    ...
    g = Graph2D(
        ...
        get_marker_size=lambda x : mpl.rcParams['lines.markersize'] ** 2 * x["rank"],
    )

    fig = plt.figure()
    ax = fig.add_subplot(1, 1, 1)
    moreGeneralGraph(ax,g)
    plt.show()  
...
def investigateLoadsAgainstTopXofBestYRankedByZ2(dfs,x, y,lowIsGoodY, z, lowIsGoodZ, numMarkers, dispatchNo, dispatchTitle):
    # hardcoding dispatch 7 for now!
    # take the ranking FIRST
    ranked = rankBy(dfs,(dispatchNo,1),z,lowIsGoodZ)
    #def rankBy(dfs,id,by,lowIsGood, cols = [] ):
...
def main():
    ...
    #threeByThree(dfs)
    investigateLoadsAgainstTopXofBestYRankedByZ(dfs,5,"Space Needed in L1",False,"Kernel Time",True,True,1,"Dispatch 1\nmatvec: <1x400>, <1200x400> -> <1x1200>")
    investigateLoadsAgainstTopXofBestYRankedByZ(dfs,5,"Space Needed in L1",False,"Kernel Time",True,True,8,"Dispatch 8\nmatvec: <1x600>, <600x600> -> <1x600>")
    investigateLoadsAgainstTopXofBestYRankedByZ(dfs,5,"Space Needed in L1",False,"Kernel Time",True,True,7,"Dispatch 7\nmatvec: <1x400>, <600x400> -> <1x600>")
    
    # titles = ["Dispatch 1\nmatvec: <1x400>, <1200x400> -> <1x1200>","Dispatch 8\nmatvec: <1x600>, <600x600> -> <1x600>","Dispatch 7\nmatvec: <1x400>, <600x400> -> <1x600>"]
    
    #graphXvsYrankedbyZ(dfs,"Streaming Loads","loads","Regular Loads","loads", "Kernel Time",True,dispNo,dispTitle)
    # for dispNo, dispTitle in zip([1,8,7], titles):
    #     graphXvsYrankedbyZtopQ(dfs,"Streaming Loads","loads","Regular Loads","loads", "Kernel Time",True,dispNo,dispTitle,5)
