...
python3 myrtle.py "main\$async_dispatch_0_matmul_transpose_b_40x120x20_f64" $mode "test_output-disp-0-$mode.json" $fp
    # 8x672x672
rm -f "test_output-disp-0-$mode.json" 2> /dev/null
echo "{}" > "test_output-disp-0-$mode.json"
fp="$dir/8x672x672wm-n-k-fakeNN-graphing.csv" # custom dispatch's search space
python3 myrtle.py "main\$async_dispatch_0_matmul_transpose_b_8x672x672_f64" $mode "test_output-disp-0-$mode.json" $fp
    # 56 x 56 x 56
rm -f "test_output-disp-0-$mode.json" 2> /dev/null
echo "{}" > "test_output-disp-0-$mode.json"
fp="$dir/56x56x56wm-n-k-fakeNN-graphing.csv" # custom dispatch's search space
python3 myrtle.py "main\$async_dispatch_0_matmul_transpose_b_56x56x56_f64" $mode "test_output-disp-0-$mode.json" $fp
    # 120x40x20
rm -f "test_output-disp-0-$mode.json" 2> /dev/null
echo "{}" > "test_output-disp-0-$mode.json"
fp="$dir/120x40x20wm-n-k-fakeNN-graphing.csv" # custom dispatch's search space
python3 myrtle.py "main\$async_dispatch_0_matmul_transpose_b_120x40x20_f64" $mode "test_output-disp-0-$mode.json" $fp
